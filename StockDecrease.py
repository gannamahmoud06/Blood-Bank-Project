...
def display_StockDecrease():
    ################################## Center Frame #####################################################
    StockDecrease_root = Tk()
    StockDecrease_root.title("Stock Decrease")
    w = 800
    h = 600
    screenwidth = StockDecrease_root.winfo_screenwidth()
    screenheight = StockDecrease_root.winfo_screenheight()
    x = int((screenwidth - w) / 2)
    y = int((screenheight - h) / 2)
    StockDecrease_root.geometry(f'{w}x{h}+{x}+{y}')
    StockDecrease_root.resizable(False, False)

    StockDecrease_root.configure(bg="#6e1423")
    StockDecrease_root.iconbitmap("project pictures/Blood group.ico")

    lbl = Label(StockDecrease_root, text='Stock (Decrease)', font=(
        "Cambria", 40, "bold", "italic"), bg="#6e1423", fg="#f8e5ee")
    lbl.pack(pady=20)

    main_frame = Frame(StockDecrease_root, bg="#6e1423")
    ...
    # Frame to hold update button
    update_frame = Frame(StockDecrease_root, bg="#6e1423")
    ...
    StockDecrease_root.tree = ttk.Treeview(
        StockDecrease_root, columns=('blood group', 'units'), show='headings')
    StockDecrease_root.tree.heading('blood group', text='Blood Group')
    StockDecrease_root.tree.heading('units', text='Units')
    StockDecrease_root.tree.pack(pady=10, padx=10, fill=tk.BOTH, expand=True)

    load_data()

    # Frame to hold the buttons at the bottom
    button_frame = Frame(StockDecrease_root, bg="#6e1423")
    ...
    close_btn = Button(button_frame, text='close', font=(
        "bold"), width=9, bg="#003049", fg="white", command=StockDecrease_root.destroy)
    close_btn.pack(side=RIGHT, padx=10)

    # Start the main loop
    StockDecrease_root.mainloop()


if __name__ == "__main__":
    display_StockDecrease()
